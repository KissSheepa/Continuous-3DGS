...
from utils.cfccell_utils import CfCCell
...
class DeformNetwork(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False):
        ...
        self.embed_hx, hx_input_ch = get_embedder(2, 32)
        self.input_ch = xyz_input_ch + time_input_ch
        self.cfc = CfCCell(
            input_size=93 if is_blender else 84,  # h [N,93]
            hidden_size=256,
            mode="default",
    ...
    def forward(self, x, t, ts, gaussians):
        t_emb = self.embed_time_fn(t)
        if self.is_blender:
            t_emb = self.timenet(t_emb)  # better for D-NeRF Dataset
        x_emb = self.embed_fn(x)
        hx_emb = self.embed_hx(gaussians._hx)
        h = torch.cat([x_emb, t_emb], dim=-1)
        # h, _ = self.cfc(h, torch.zeros_like(hx_emb), ts)
        h, _ = self.cfc(h, hx_emb, ts)
        # h, _ = self.cfc(h, gaussians._hx, ts)
        if self.is_6dof:
            w = self.branch_w(h)
            v = self.branch_v(h)
